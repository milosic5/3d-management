  ...
  <AppLayout>
    <template #breadcrumb>
        <div class="flex items-center space-x-2">
            <Link :href="route('orders.index')" class="hover:underline">{{ $t('orders.title') }}</Link>
            <span>/</span>
            <span class="text-slate-800 dark:text-slate-200 font-medium">{{ $t('orders.new') }}</span>
        </div>
    </template>
    
    <PageHeader :title="$t('orders.new')" description="" />
    ...
    <form @submit.prevent="submit" class="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
      ...
      <div class="xl:col-span-2 space-y-6">
        ...
        <Card class="p-0 overflow-visible">
            <div class="p-6 border-b flex items-center justify-between rounded-t-xl">
            ...
            <div class="divide-y">
                <div v-for="(item, i) in form.items" :key="i" class="p-6 bg-slate-50/50 dark:bg-slate-900/50 last:rounded-b-xl">
      ...
      <div class="xl:col-span-1 space-y-6 sticky top-24">
        <Card class="p-6">
            ...
            <Button type="submit" class="w-full mt-6 bg-slate-900 text-white hover:bg-slate-800" :disabled="form.processing">
                {{ form.processing ? $t('common.loading') : $t('orders.create_order') }}
            </Button>
        </Card>
      </div>
    </form>
...
const form = useForm({
    ...
    items: [
        { product_id: '', filament_id: '', quantity: 1, unit_price: 0, print_cost: 0, print_time_minutes: 0, color_name: '', color_hex: '', notes: '', _base_unit_cost: 0 }
    ]
})
