  ...
  <AppLayout>
    <template #breadcrumb>
        <div class="flex items-center space-x-2">
            ...
            <span class="text-slate-800 dark:text-slate-200 font-medium">{{ $t('calibrations.edit') }}</span>
        </div>
    </template>
    
    <PageHeader :title="$t('calibrations.edit')" description="" />
      ...
      <Card class="p-6 space-y-6">
        ...
        <div class="flex justify-end space-x-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <Button class="bg-orange-500 hover:bg-orange-600 text-white" type="submit" :disabled="form.processing">
                 {{ form.processing ? $t('common.loading') : $t('calibrations.edit') }}
            </Button>
        </div>
      </Card>
    </form>
...
const form = useForm({
    filament_id: props.calibration.filament_id,
    temperature: props.calibration.temperature,
    flow_ratio: props.calibration.flow_ratio,
    pressure_advance: props.calibration.pressure_advance,
    max_volumetric_speed: props.calibration.max_volumetric_speed
})

const submit = () => {
    form.put(route('calibrations.update', props.calibration.id))
}
</script>
