  ...
  <AppLayout>
    <template #breadcrumb>
        <div class="flex items-center space-x-2">
            ...
            <span class="text-slate-500">{{ filament.brand }} {{ filament.name }}</span>
            <span class="text-slate-400">/</span>
            <span class="text-slate-800 dark:text-slate-200 font-medium">{{ $t('filaments.edit') }}</span>
        </div>
    </template>
    
    <PageHeader :title="$t('filaments.edit')" description="" />
...
const form = useForm({
    brand: props.filament.brand,
    name: props.filament.name,
    material: props.filament.material,
    color_name: props.filament.color_name,
    color_hex: props.filament.color_hex,
    price_per_kg: props.filament.price_per_kg,
    empty_spool_weight_grams: props.filament.empty_spool_weight_grams,
    notes: props.filament.notes || ''
})



const submit = () => {
    form.put(route('filaments.update', props.filament.id))
}
</script>
