  ...
  <AppLayout>
    <template #breadcrumb>
        <div class="flex items-center space-x-2">
            ...
            <span class="text-slate-400">/</span>
            <span class="text-slate-800 dark:text-slate-200 font-medium">{{ $t('products.edit') }}</span>
        </div>
    </template>
    
    <PageHeader :title="$t('products.edit')" description="" />
      ...
      <Card class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div v-else-if="product.image_path" class="mb-4">
                    <img :src="`/storage/${product.image_path}`" class="w-32 h-32 object-cover rounded-md border" />
                </div>
                <FilePicker accept="image/*" label="Upload new photo (replaces old)" @change="onImageChange">
                    <template #icon>
...
const props = defineProps({ product: Object, filaments: Array })
...
const form = useForm({
    ...
    print_time_minutes: props.product.print_time_minutes,
